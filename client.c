...
void send_data_chunks(int sockfd,struct sockaddr_in *server,char *send_data,int data_len)
{
    ...
    for(int i=0;i<total_no_of_chunks;i++)
    {
        ...
        sendto(sockfd,&data_packet,sizeof(data_packet),0,(struct sockaddr *)server,addr_len);
    }
    ...
    while(no_of_acks<total_no_of_chunks)
    {
        ...
        {
            if(recvfrom(sockfd,&ack_packet,sizeof(ack_packet),0,(struct sockaddr *)server,&addr_len)>0 && ack_packet.seq_num<total_no_of_chunks)
            {
                if(acked_chunks[ack_packet.seq_num]==0)
                {
                    acked_chunks[ack_packet.seq_num]=1;
                    printf("ACK recieved for %d chumk\n",ack_packet.seq_num+1);
                    no_of_acks++;
                }
            }
        }
...
void receive_data_chunks(int sockfd,struct sockaddr_in *server)
{
    packet data_packet;
    ack ack_packet;
    ...
    while(1)
    {
        if(recvfrom(sockfd,&data_packet,sizeof(data_packet),0,(struct sockaddr *)server,&addr_len)>0)
        {
            ...
            if(data_packet.seq_num<total_chunks)
            {
                ...
                sendto(sockfd,&ack_packet,sizeof(ack_packet),0,(struct sockaddr *)server,addr_len);
            }
            ...
            if(recv_chunks==total_chunks)
            {
                ...
                if(strncmp(received_data,"exit",4)==0)
                    end=1;
                break;
            }
        }
...
int main()
{
    ...
    while(1)
    {
        ...
        send_data_chunks(sockfd,&server,input,strlen(input));
        if(strncmp(input,"exit",4)==0)
            break;
        receive_data_chunks(sockfd,&server);
        if(end==1)
            break;
    }
    close(sockfd);
